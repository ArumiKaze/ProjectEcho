...
ANohCharacter::ANohCharacter()
{
	...
	b_isattacking = false;
	b_combatmode = false;
	b_isledgeclimbing = false;

	//Sub character states
	b_canmove = true;
	b_ischaining = false;
	b_isswitching_camerazoom = false;
	b_foundweapon = false;
	b_canledgetrace = false;
	combonumber = 0;
	currentweaponindex = 0;
	weaponswapcasttime = 2.0f;



	/*
	PrimaryActorTick.bCanEverTick = true;
	spheretracerledge = CreateDefaultSubobject<USphereComponent>(TEXT("SphereTracerLedge"));
	spheretracerledge->SetupAttachment(RootComponent);
	spheretracerledge->InitSphereRadius(100.0f);
	spheretracerledge->SetCollisionProfileName(TEXT("SphereTracer"));
	spheretracerledge->SetCollisionObjectType(ECC_GameTraceChannel1);
	spheretracerledge->SetCollisionResponseToChannel(ECC_GameTraceChannel2, ECR_Block);
	spheretracerledge->SetCollisionResponseToChannel(ECC_WorldStatic, ECR_Overlap);
	spheretracerledge->OnComponentBeginOverlap.AddDynamic(this, &ANohCharacter::OnOverlapBegin);
	spheretracerledge->OnComponentEndOverlap.AddDynamic(this, &ANohCharacter::OnOverlapEnd);
	//ECC_GameTraceChannel1 == SphereTracer
	//ECC_GameTraceChannel2 == LedgeTracer
	*/
...
void ANohCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)		//Set up input bindings that call functions
{
	...
	PlayerInputComponent->BindAction("Cancel", IE_Pressed, this, &ANohCharacter::Cancel);
}
...
//
void ANohCharacter::Cancel()
{
	if (b_isledgeclimbing)
	{
		LedgeLetGo();
	}
}
...
/*
//Dodge skill//
void ANohCharacter::Dodge()
{
	if (!b_isdodging && !GetCharacterMovement()->IsFalling() && !b_isattacking && !b_isswitching && GetVelocity().Size() > 100.0f)
	{
		b_isdodging = true;
		b_canmove = false;
...
//Called in Tick when character is dodging
void ANohCharacter::DodgeTick(float delta)
{
	if (!GetCharacterMovement()->IsFalling())
	{
		dodgespeed_x = FMath::FInterpTo(dodgespeed_x, 0.0f, delta, 1.0f);
		dodgespeed_y = FMath::FInterpTo(dodgespeed_y, 0.0f, delta, 1.0f);
		FVector Force = FVector(GetActorForwardVector().X * dodgespeed_x, GetActorForwardVector().Y * dodgespeed_y, def_dropgravity);
		LaunchCharacter(Force, false, false);
	}
}
//Reset after dodge cooldown is finished//
void ANohCharacter::DodgeCooldown()
{
	dodgespeed_x = def_dodgespeed;
	dodgespeed_y = def_dodgespeed;
...
//Weapon swap skill//
void ANohCharacter::WeaponSwitchHold()
{
	if (!b_isswitching && !b_isswitching_camerazoom && !b_issheathing && !GetCharacterMovement()->IsFalling() && !b_isdodging && !b_isattacking)
	{
		if (!b_ischargingswitch)
		{
			b_ischargingswitch = true;
			GetWorld()->GetTimerManager().SetTimer(weaponswap_timer, this, &ANohCharacter::WeaponSwitchAction, weaponswapcasttime);
		}
		else if(!b_isswitching)
		{
			b_ischargingswitch = false;
			GetWorld()->GetTimerManager().ClearTimer(weaponswap_timer);
		}
	}
}
//Called by WeaponSwitchHold when weapon swap key is held down long enough
void ANohCharacter::WeaponSwitchAction()
{
	if (!b_isswitching)
	{
		b_ischargingswitch = false;
		...
		b_isswitching_camerazoom = true;
		hud->hud_weaponswitch(b_isswitching);
	}
	else if(b_isswitching && !b_isswitching_camerazoom)
	{
		b_isswitching = false;
		NohController->bShowMouseCursor = false;
		NohController->SetInputMode(FInputModeGameOnly());
		b_isswitching_camerazoom = true;
		hud->hud_weaponswitch(b_isswitching);
	}
}
//Called every tick when entering or exiting weapon swap mode, updates player camera to new location/rotation
void ANohCharacter::weaponswitchtimeline(float delta)
{
	if (b_isswitching)
	{
		float camera_x{ FMath::FInterpTo(FollowCamera->RelativeLocation.X, 255.0f, delta, 2.0f) };
		float camera_y{ FMath::FInterpTo(FollowCamera->RelativeLocation.Y, 144.0f, delta, 2.0f) };
		float camera_z{ FMath::FInterpTo(FollowCamera->RelativeLocation.Z, 68.0f, delta, 2.0f) };

		float camera_roll{ FMath::FInterpTo(FollowCamera->RelativeRotation.Roll, 0.0f, delta, 2.0f) };
		float camera_pitch{ FMath::FInterpTo(FollowCamera->RelativeRotation.Pitch, 0.0f, delta, 2.0f) };
		float camera_yaw{ FMath::FInterpTo(FollowCamera->RelativeRotation.Yaw, -40.0f, delta, 2.0f) };

		FollowCamera->SetRelativeLocationAndRotation(FVector(camera_x, camera_y, camera_z), FRotator(camera_pitch, camera_yaw, camera_roll));
		
		if (camera_x > 245.0f && camera_yaw < -30.0f)
		{
			b_isswitching_camerazoom = false;
			NohController->bShowMouseCursor = true;
			NohController->SetInputMode(FInputModeUIOnly());
		}
	}
	else
	{
		float camera_x = FMath::FInterpTo(FollowCamera->RelativeLocation.X, 0.0f, delta, 3.0f);
		float camera_y = FMath::FInterpTo(FollowCamera->RelativeLocation.Y, 0.0f, delta, 3.0f);
		float camera_z = FMath::FInterpTo(FollowCamera->RelativeLocation.Z, 0.0f, delta, 3.0f);

		float camera_roll = FMath::FInterpTo(FollowCamera->RelativeRotation.Roll, 0.0f, delta, 3.0f);
		float camera_pitch = FMath::FInterpTo(FollowCamera->RelativeRotation.Pitch, 0.0f, delta, 3.0f);
		float camera_yaw = FMath::FInterpTo(FollowCamera->RelativeRotation.Yaw, 0.0f, delta, 3.0f);

		FollowCamera->SetRelativeLocationAndRotation(FVector(camera_x, camera_y, camera_z), FRotator(camera_pitch, camera_yaw, camera_roll));

		if (camera_x <= 10.0f && camera_yaw >= -10.0f)
		{
			b_isswitching_camerazoom = false;
			b_canmove = true;
	...
	NohController->SetControlRotation(FRotator(GetActorRotation().Pitch, GetActorRotation().Yaw, GetActorRotation().Roll));
}



//Character attack skill//
void ANohCharacter::Attack()
{
	if (!b_issheathed && !b_issheathing && !GetCharacterMovement()->IsFalling() && !b_isdodging && !b_isattacking && !b_isswitching)
	{
		if (b_ischaining)
		{
			GetWorld()->GetTimerManager().ClearTimer(combo_timer);
		}
		else
		{
			b_ischaining = true;
...
//Calls by attack animation anim notifies, starts a timer of how much time the player has left to keep combo-ing before it resets
void ANohCharacter::Combo()
{
	b_isattacking = false;
	b_canmove = true;
	GetWorld()->GetTimerManager().SetTimer(combo_timer, this, &ANohCharacter::ComboReset, 3.0f);
}
//Called by finisher attack animation or when timer runs out, resets combo
void ANohCharacter::ComboReset()
{
	b_isattacking = false;
	b_canmove = true;
	b_ischaining = false;
...
//
void ANohCharacter::LedgeWallTrace()
{
	FHitResult HitOut{ ForceInit };
	FVector Start{ GetActorLocation().X, GetActorLocation().Y, GetActorLocation().Z - 0.0f };
	FVector End{ UKismetMathLibrary::GetForwardVector(GetActorRotation()).X * 150.0f, UKismetMathLibrary::GetForwardVector(GetActorRotation()).Y * 150.0f, UKismetMathLibrary::GetForwardVector(GetActorRotation()).Z - 0.0f };
	ECollisionChannel CollisionChannel{ ECC_GameTraceChannel2 };
	FCollisionQueryParams TraceParams{ FName(TEXT("character ledgewalltrace")), true, this };
	...
	if (GetWorld()->SweepSingleByChannel(HitOut, Start, Start + End, FQuat(), CollisionChannel, FCollisionShape::MakeSphere(20.0f), TraceParams))
	{
		walltraceimpact = HitOut.ImpactPoint;
		wallnormal = HitOut.ImpactNormal;
	}
}
//
void ANohCharacter::LedgeHeightTrace()
{
	FHitResult HitOut{ ForceInit };
	FVector Start{ GetActorLocation().X, GetActorLocation().Y, GetActorLocation().Z + 500.0f };
	FVector End{ UKismetMathLibrary::GetForwardVector(GetActorRotation()) * 70.0f };
	ECollisionChannel CollisionChannel{ ECC_GameTraceChannel2 };
	FCollisionQueryParams TraceParams{ FName(TEXT("character ledgeheighttrace")), true, this };
	//TraceParams.bTraceAsyncScene = true;
	TraceParams.bReturnPhysicalMaterial = false;
	...
	if (GetWorld()->SweepSingleByChannel(HitOut, Start + End, FVector{ Start.X + End.X, Start.Y + End.Y, (Start.Z + End.Z) - 550.0f}, FQuat(), CollisionChannel, FCollisionShape::MakeSphere(20.0f), TraceParams))
	{
		ledgeheight = HitOut.ImpactPoint;

		float value{ GetMesh()->GetSocketLocation(FName(TEXT("HipSocket"))).Z - ledgeheight.Z };
		if (value >= -50.0f && value <= 0.0f)
		{
			if (!b_isledgeclimbing)
			{
				b_isledgeclimbing = true;
				GetCharacterMovement()->SetMovementMode(EMovementMode::MOVE_Flying);

				FLatentActionInfo LatentActionInfo;
				LatentActionInfo.CallbackTarget = GetCharacterMovement();
				LatentActionInfo.ExecutionFunction = "StopMovementImmediately";
				LatentActionInfo.UUID = 123;
				LatentActionInfo.Linkage = 0;

				FVector capsuleradius{ GetCapsuleComponent()->GetUnscaledCapsuleRadius(), GetCapsuleComponent()->GetUnscaledCapsuleRadius(), 0.0f };
				FVector targetlocation{ (wallnormal * capsuleradius).X + walltraceimpact.X, (wallnormal * capsuleradius).Y + walltraceimpact.Y, ledgeheight.Z - 133.0f };

				FRotator targetrotation{ 0.0f, UKismetMathLibrary::FindLookAtRotation(GetActorLocation(), walltraceimpact).Yaw, UKismetMathLibrary::FindLookAtRotation(GetActorLocation(), walltraceimpact).Roll };

				UKismetSystemLibrary::MoveComponentTo(GetCapsuleComponent(), targetlocation, targetrotation, false, false, 0.2f, true, EMoveComponentAction::Type::Move, LatentActionInfo);
			}
		}
	}
}

void ANohCharacter::LedgeLetGo()
{
	b_isledgeclimbing = false;
...
//
void ANohCharacter::OnOverlapBegin(UPrimitiveComponent * OverlapComponent, AActor * OtherActor, UPrimitiveComponent * OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult & SweepResult)
{
	b_canledgetrace = true;
}
//
void ANohCharacter::OnOverlapEnd(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex)
{
	b_canledgetrace = false;
}
...
//Adds newly obtained weapon into inventory//
void ANohCharacter::addWeaponInventory(AWeapons* w)
{
	weapon_inventory.Emplace(w);
}
...
{
	for (int i{ 0 }; i < weapon_inventory.Num(); ++i)
	{
		if (weapon_inventory[i]->getweaponType() == x)
		{
			b_foundweapon = true;
			weapon_inventory[currentweaponindex]->Destroy();
	...
	{
		FAttachmentTransformRules rules(EAttachmentRule::SnapToTarget, true);
		weaponsocket = TEXT("weaponholster_back");
		weapon_inventory[currentweaponindex]->AttachToComponent(GetMesh(), rules, weaponsocket);
	}
}
...
{
	return weapon_inventory[currentweaponindex];
}
